...
const LIVE_ORDER_AUDIT_TTL_SECONDS = 7 * 24 * 60 * 60

type LiveProtectionState = "pending" | "deferred" | "unprotected" | "partial" | "armed"
...
interface LiveOrderAudit {
  ...
  protectionState: LiveProtectionState
  createdAt: string
  updatedAt: string
}
...
async function persistLiveOrderAudit(audit: LiveOrderAudit): Promise<void> {
  try {
    ...
    const now = Date.now()
    audit.updatedAt = new Date(now).toISOString()
    const itemKey = `live_order_audit:${audit.connectionId}:${audit.traceId}`
    const recentKey = `live_order_audit:${audit.connectionId}:recent`
    await Promise.all([
      client.setex(itemKey, LIVE_ORDER_AUDIT_TTL_SECONDS, JSON.stringify(audit)),
      client.zadd(recentKey, now, audit.traceId),
      client.zremrangebyscore(recentKey, "-inf", now - LIVE_ORDER_AUDIT_TTL_SECONDS * 1000),
      client.expire(recentKey, LIVE_ORDER_AUDIT_TTL_SECONDS),
    ])
  } catch {
    // Audit telemetry must never interrupt live order execution.
  }
}
...
export async function executeLivePosition(
  ...
  try {
    ...
    if (computedVolume <= 0 || !Number.isFinite(computedVolume)) {
      // Resolve the venue's current public contract rule even when the main
      // calculator failed. Only fall back to $5 when the exchange metadata
      // endpoint itself is unavailable.
      const FALLBACK_NOTIONAL_USD = 5
      try {
        const { resolveExchangeMinimum } = await import("@/lib/exchange-contract-rules")
        const minimumRule = await resolveExchangeMinimum(connSettings.exchange, realPosition.symbol, currentPrice)
        computedVolume = minimumRule.quantity
        volumeNote = ` [synthesized-min: exchange rule ${minimumRule.source} — calculator returned ${volumeResult?.finalVolume ?? "null"}]`
      } catch {
        computedVolume = currentPrice > 0 ? FALLBACK_NOTIONAL_USD / currentPrice : 0
        volumeNote = ` [synthesized-min: $${FALLBACK_NOTIONAL_USD} emergency fallback — exchange rule unavailable]`
      }
      await logProgressionEvent(
        connectionId,
        "live_trading",
    ...
    const liveOrderAudit: LiveOrderAudit = {
      ...
      symbol: realPosition.symbol,
      direction: realPosition.direction,
      requestedQuantity: computedVolume,
      entryOrderStatus: "attempting",
    ...
    if (!orderResult?.success && isNonRecoverableExchangeError(orderResult)) {
      ...
      if (reducedLev < livePosition.leverage) {
        ...
        if (retryResult?.success && (retryResult.orderId || retryResult.id)) {
          ...
          if (!(minQtyForSymbol > 0)) {
            try {
              const { resolveExchangeMinimum } = await import("@/lib/exchange-contract-rules")
              const minimumRule = await resolveExchangeMinimum(freshSettings.exchange, realPosition.symbol, currentPrice)
              minQtyForSymbol = minimumRule.quantity
            } catch {
              minQtyForSymbol = currentPrice > 0 ? 5 / currentPrice : 0
            }
          }
    ...
    liveOrderAudit.entryOrderStatus = "placed"
    ...
    if (fill.filled && fill.filledQty > 0) {
      ...
      liveOrderAudit.executedQuantity = fill.filledQty
      liveOrderAudit.averageExecutionPrice = fill.filledPrice || currentPrice
      await persistLiveOrderAudit(liveOrderAudit)
      ...
      liveOrderAudit.entryOrderStatus = deferredStatus
      liveOrderAudit.protectionState = "deferred"
      await persistLiveOrderAudit(liveOrderAudit)
      livePosition.statusReason =
    ...
    if (livePosition.executedQuantity > 0) {
      ...
      livePosition.protectionState = slOrderId && tpOrderId
        ? "armed"
        : slOrderId || tpOrderId
          ? "partial"
          : "unprotected"
      liveOrderAudit.stopLossOrderId = slOrderId || undefined
      liveOrderAudit.takeProfitOrderId = tpOrderId || undefined
      liveOrderAudit.protectionState = livePosition.protectionState
      await persistLiveOrderAudit(liveOrderAudit)
