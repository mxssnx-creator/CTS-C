...
export class ProgressionLimitsManager {
  ...
  private getDefaultLimits(): ProgressionLimits {
    return {
      ...
      combined: {
        // 0 means unlimited globally. Real exchange/per-connection limits and
        // risk checks still apply; progression must not invent a global cap.
        maxOpenPositions: 0,
        maxDrawdown: -0.2, // -20%
  ...
  async validateTradeExecution(
    ...
    try {
      ...
      if (limits.combined.maxOpenPositions > 0 && openCount >= limits.combined.maxOpenPositions) {
        return {
          available: false,
