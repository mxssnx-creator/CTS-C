...
export class BingXConnector extends BaseExchangeConnector {
  ...
  async modifyPosition(
    ...
    try {
      ...
      if (marginType) results.push(await this.setMarginType(symbol, marginType))
      if (leverage) results.push(await this.setLeverage(symbol, leverage))
      const failed = results.find((result) => !result.success)
      if (failed) throw new Error(failed.error || "Position modification failed")
  ...
  async setPositionMode(hedgeMode: boolean): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const { signature, queryString: signedQs } = this.signParams(params)
      // BingX exposes both query and update on the v1 position-mode route.
      const url = `${this.getBaseUrl()}/openApi/swap/v1/positionSide/dual?${signedQs}&signature=${signature}`
  ...
  async getPositionMode(): Promise<{ success: boolean; hedgeMode?: boolean; error?: string }> {
    try {
      await this.syncServerTime()
      const { signature, queryString: signedQs } = this.signParams({ timestamp: this.getTimestamp() })
      const url = `${this.getBaseUrl()}/openApi/swap/v1/positionSide/dual?${signedQs}&signature=${signature}`
      const response = await this.rateLimitedFetch(url, {
        method: "GET",
        headers: { "X-BX-APIKEY": this.credentials.apiKey },
      ...
      const data = await this.safeJson(response)
      if (!response.ok || !this.isBingXSuccess(data.code)) {
        throw new Error(`BingX API error (code=${data.code}): ${data.msg || `HTTP ${response.status}`}`)
      }
      const raw = data.data?.dualSidePosition ?? data.data?.dualSide ?? data.data
      const hedgeMode = raw === true || raw === "true" || raw === 1 || raw === "1"
      return { success: true, hedgeMode }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error)
      this.logError(`✗ Failed to query position mode: ${errorMsg}`)
      return { success: false, error: errorMsg }
    }
  }
