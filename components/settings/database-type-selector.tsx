  ...
  return (
    <Card className="p-4">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        The system uses a Redis-compatible in-process store with atomic disk snapshots.
      </p>
      <div className="flex items-center gap-2 bg-primary/10 p-3 rounded-md">
        <div className="w-3 h-3 bg-green-500 rounded-full"></div>
        <span className="font-medium">Inline Redis + durable snapshot</span>
      </div>
      <p className="text-xs text-muted-foreground mt-4">
        Production uses one long-running instance and a mounted snapshot volume.
      </p>
    </Card>
  )
}
